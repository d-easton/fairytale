...
EnglishLengthChart.prototype.update = (storyLength, lengthScale) => {
    var self = this;

    const selector = "#english-length-svg";
    const lC = d3.select(selector).selectAll("rect").data(storyLength);
        lC.exit().remove();
        lC.enter()
            .append("rect")
                .attr("class", "english-length-rectangle")
            .merge(lC)
                .attr("height", "50")
